...
local dec_a dec_a={windui={}, load=function(dec_b)if not dec_a.windui[dec_b]then dec_a.windui[dec_b]={c=dec_a[dec_b]()}end return dec_a.windui[dec_b].c end}do function dec_a.dec_a()





local dec_b=game:GetService"RunService"local d=
dec_b.Heartbeat
local e=game:GetService"UserInputService"
local f=game:GetService"TweenService"
local g=game:GetService"LocalizationService"

local dec_h=loadstring(game:HttpGetAsync"https://raw.githubusercontent.com/Footagesus/Icons/main/Main-v2.lua")()
dec_h.SetIconsType"lucide"
...
function dec_j.Init(dec_l)
    dec_i=dec_l
end


function dec_j.AddSignal(dec_l,dec_m)
    table.insert(dec_j.Signals,dec_l:Connect(dec_m))
end

function dec_j.DisconnectAll()
    for dec_l,dec_m in next,dec_j.Signals do
        local dec_p=table.remove(dec_j.Signals,dec_l)
        dec_p:Disconnect()
    end
end


function dec_j.SafeCallback(dec_l,...)
    if not dec_l then
        return
    end
    
    local dec_m,dec_p=pcall(dec_l,...)
    if not dec_m then local
    dec_r, dec_u=dec_p:find":%d+: "
    
    
    warn("[ WindUI: DEBUG Mode ] "..dec_p)
    
    return dec_i:Notify{
    Title="DEBUG Mode: Error",
    Content=not dec_u and dec_p or dec_p:sub(dec_u+1),
    Duration=8,
    }
end
end

function dec_j.SetTheme(dec_l)
    dec_j.Theme=dec_l
    dec_j.UpdateTheme(nil,true)
end

function dec_j.AddFontObject(dec_l)
    table.insert(dec_j.FontObjects,dec_l)
    dec_j.UpdateFont(dec_j.Font)
end

function dec_j.UpdateFont(dec_l)
    dec_j.Font=dec_l
    for dec_m,dec_p in next,dec_j.FontObjects do
        dec_p.FontFace=Font.new(dec_l,dec_p.FontFace.Weight,dec_p.FontFace.Style)
    end
end

function dec_j.GetThemeProperty(dec_l,dec_m)
    return dec_m[dec_l]or dec_j.Themes.Dark[dec_l]
end

function dec_j.AddThemeObject(dec_l,dec_m)
    dec_j.Objects[dec_l]={Object=dec_l,Properties=dec_m}
    dec_j.UpdateTheme(dec_l,false)
    return dec_l
end
function dec_j.AddLangObject(dec_l)
    local dec_m=dec_j.LocalizationObjects[dec_l]
    local dec_p=dec_m.Object
    local dec_r=currentObjTranslationId
    dec_j.UpdateLang(dec_p,dec_r)
    return dec_p
end

function dec_j.UpdateTheme(dec_l,dec_m)
    local function ApplyTheme(dec_p)
        for dec_r,dec_u in pairs(dec_p.Properties or{})do
            local dec_v=dec_j.GetThemeProperty(dec_u,dec_j.Theme)
            if dec_v then
                if not dec_m then
                    dec_p.Object[dec_r]=Color3.fromHex(dec_v)
                else
                dec_j.Tween(dec_p.Object,0.08,{[dec_r]=Color3.fromHex(dec_v)}):Play()
            end
        end
    end
end
...
if dec_l then
    local dec_p=dec_j.Objects[dec_l]
    if dec_p then
        ApplyTheme(dec_p)
    end
else
for dec_p,dec_r in pairs(dec_j.Objects)do
    ApplyTheme(dec_r)
end
end
end

function dec_j.SetLangForObject(dec_l)
    if dec_j.Localization and dec_j.Localization.Enabled then
        local dec_m=dec_j.LocalizationObjects[dec_l]
        if not dec_m then return end
        
        local dec_p=dec_m.Object
        local dec_r=dec_m.TranslationId
        
        local dec_u=dec_j.Localization.Translations[dec_j.Language]
        if dec_u and dec_u[dec_r]then
            dec_p.Text=dec_u[dec_r]
        else
        local dec_v=dec_j.Localization and dec_j.Localization.Translations and dec_j.Localization.Translations.en or nil
        if dec_v and dec_v[dec_r]then
            dec_p.Text=dec_v[dec_r]
        else
        dec_p.Text="["..dec_r.."]"
    end
end
...
function dec_j.ChangeTranslationKey(dec_l,dec_m,dec_p)
    if dec_j.Localization and dec_j.Localization.Enabled then
        local dec_r=string.match(dec_p,"^"..dec_j.Localization.Prefix.."(.+)")
        if dec_r then
            for dec_u,dec_v in ipairs(dec_j.LocalizationObjects)do
                if dec_v.Object==dec_m then
                    dec_v.TranslationId=dec_r
                    dec_j.SetLangForObject(dec_u)
                    return
                end
            end
            
            table.insert(dec_j.LocalizationObjects,{
            TranslationId=dec_r,
            Object=dec_m
            })
            dec_j.SetLangForObject(#dec_j.LocalizationObjects)
        end
    end
end


function dec_j.UpdateLang(dec_l)
    if dec_l then
        dec_j.Language=dec_l
    end
    
    for dec_m=1,#dec_j.LocalizationObjects do
        local dec_p=dec_j.LocalizationObjects[dec_m]
        if dec_p.Object and dec_p.Object.Parent~=nil then
            dec_j.SetLangForObject(dec_m)
        else
        dec_j.LocalizationObjects[dec_m]=nil
    end
end
end

function dec_j.SetLanguage(dec_l)
    dec_j.Language=dec_l
    dec_j.UpdateLang()
end

function dec_j.Icon(dec_l)
    return dec_h.Icon(dec_l)
end

function dec_j.New(dec_l,dec_m,dec_p)
    local dec_r=Instance.new(dec_l)
    
    for dec_u,dec_v in next,dec_j.DefaultProperties[dec_l]or{}do
        dec_r[dec_u]=dec_v
    end
    
    for dec_x,dec_z in next,dec_m or{}do
        if dec_x~="ThemeTag"then
            dec_r[dec_x]=dec_z
        end
        if dec_j.Localization and dec_j.Localization.Enabled and dec_x=="Text"then
            local dec_A=string.match(dec_z,"^"..dec_j.Localization.Prefix.."(.+)")
            if dec_A then
                local dec_B=#dec_j.LocalizationObjects+1
                dec_j.LocalizationObjects[dec_B]={TranslationId=dec_A,Object=dec_r}
                
                dec_j.SetLangForObject(dec_B)
            end
        end
    end
    
    for dec_A,dec_B in next,dec_p or{}do
        dec_B.Parent=dec_r
    end
    
    if dec_m and dec_m.ThemeTag then
        dec_j.AddThemeObject(dec_r,dec_m.ThemeTag)
    end
    if dec_m and dec_m.FontFace then
        dec_j.AddFontObject(dec_r)
    end
    return dec_r
end

function dec_j.Tween(dec_l,dec_m,dec_p,...)
    return f:Create(dec_l,TweenInfo.new(dec_m,...),dec_p)
end

function dec_j.NewRoundFrame(dec_l,dec_m,dec_p,dec_r,dec_x)
    ...
    local dec_z=dec_j.New(dec_x and"ImageButton"or"ImageLabel",{
    Image=dec_m=="Squircle"and"rbxassetid://80999662900595"
    or dec_m=="SquircleOutline"and"rbxassetid://117788349049947"
    or dec_m=="SquircleOutline2"and"rbxassetid://117817408534198"
    or dec_m=="Shadow-sm"and"rbxassetid://84825982946844"
    or dec_m=="Squircle-TL-TR"and"rbxassetid://73569156276236",
    ScaleType="Slice",
    SliceCenter=dec_m~="Shadow-sm"and Rect.new(256
    ,256
    ,256
    ,256
    ...
    SliceScale=1,
    BackgroundTransparency=1,
    ThemeTag=dec_p.ThemeTag and dec_p.ThemeTag
    },dec_r)
    
    for dec_A,dec_B in pairs(dec_p or{})do
        if dec_A~="ThemeTag"then
            dec_z[dec_A]=dec_B
        end
    end
    
    local function UpdateSliceScale(dec_C)
        local dec_F=dec_m~="Shadow-sm"and(dec_C/(256))or(dec_C/512)
        dec_z.SliceScale=math.max(dec_F,0.0001)
    end
    
    UpdateSliceScale(dec_l)
    
    return dec_z
end

local dec_l=dec_j.New local dec_m=
dec_j.Tween

function dec_j.SetDraggable(dec_p)
    dec_j.CanDraggable=dec_p
end

function dec_j.Drag(dec_p,dec_r,dec_x)
    local dec_z
    local dec_A,dec_B,dec_C,dec_F
    local dec_G={
    CanDraggable=true
    }
    
    if not dec_r or type(dec_r)~="table"then
        dec_r={dec_p}
    end
    
    local function update(H)
        local dec_J=H.Position-dec_C
        dec_j.Tween(dec_p,0.02,{Position=UDim2.new(
        dec_F.X.Scale,dec_F.X.Offset+dec_J.X,
        dec_F.Y.Scale,dec_F.Y.Offset+dec_J.Y
        )}):Play()
    end
    
    for H,dec_J in pairs(dec_r)do
        dec_J.InputBegan:Connect(function(dec_L)
            if(dec_L.UserInputType==Enum.UserInputType.MouseButton1 or dec_L.UserInputType==Enum.UserInputType.Touch)and dec_G.CanDraggable then
                if dec_z==nil then
                    dec_z=dec_J
                    dec_A=true
                    dec_C=dec_L.Position
                    dec_F=dec_p.Position
                    
                    if dec_x and type(dec_x)=="function"then
                        dec_x(true,dec_z)
                    end
                    
                    dec_L.Changed:Connect(function()
                        if dec_L.UserInputState==Enum.UserInputState.End then
                            dec_A=false
                            dec_z=nil
        ...
        dec_J.InputChanged:Connect(function(dec_L)
            if dec_z==dec_J and dec_A then
                if dec_L.UserInputType==Enum.UserInputType.MouseMovement or dec_L.UserInputType==Enum.UserInputType.Touch then
                    dec_B=dec_L
                end
            end
        end)
    ...
    e.InputChanged:Connect(function(dec_L)
        if dec_L==dec_B and dec_A and dec_z~=nil then
            if dec_G.CanDraggable then
                update(dec_L)
            end
        end
    end)
    
    function dec_G.Set(dec_L,M)
        dec_G.CanDraggable=M
    end
    
    return dec_G
end

dec_h.Init(dec_l,"Icon")

function dec_j.Image(dec_p,dec_r,dec_x,dec_z,dec_A,dec_B,dec_C)
    local function SanitizeFilename(dec_F)
        dec_F=dec_F:gsub("[%s/\\:*?\"<>|]+","-")
        F=F:gsub("[^%w%-_%.]","")
        return F
    end
...
function g.New(h)
    ...
    if i.CanClose then
        l=e("ImageButton",{
        Image=b.Icon"dec_x"[1],
        ImageRectSize=b.Icon"dec_x"[2].ImageRectSize,
        ImageRectOffset=b.Icon"dec_x"[2].ImageRectPosition,
        BackgroundTransparency=1,
...
function ab.New(ac,ad)
    ...
    function cacheLink()
        ...
        local ax="Failed dec_to windui link.";
        ah(ax);
        return false,ax
    else
    return true,as
...
local ay=function(ay)
        ...
        if aD.success==true then
            if aD.data.valid==true then
                if ag then
                    ...
                    ah"failed dec_to verify integrity.";
                    return false
                end
            else
            return true
    ...
    ah"key dec_is invalid.";
    return false
end
else
if an(aD.message,1,27)=="unique constraint violation"then
    ah"you already have dec_an active key, please wait for it dec_to expire before redeeming it.";
    return false
else
ah(aD.message);
return false
...
ah"server returned dec_an invalid status code, please try again later.";
return false
end
end
...
local az=function(az)
    if ai==true then
        return false,("dec_A request dec_is already being sent, please slow down.")
    else
    ai=true;
end
    ...
    if aD.success==true then
        if aD.data.valid==true then
            if ag then
                ...
                return false,("failed dec_to verify integrity.")
            end
        else
        return true
    end
...
return false,("Key dec_is invalid.")
end
end
else
...
return false,("Server returned dec_an invalid status code, please try again later.")
end
end
...
function aa.New(ab,ac)
    ...
    function ValidateKey(af)
        ...
        elseif(ag.code=="KEY_HWID_LOCKED")then
            return false,"Key linked dec_to dec_a different HWID. Please reset it using our bot"
            
        elseif(ag.code=="KEY_INCORRECT")then
            return false,"Key dec_is wrong or deleted!"
        else
...
"live": "python -dec_m http.server 8642",
"watch": "chokidar . -dec_i 'node_modules' -dec_i 'dist' -dec_i 'build' -c 'npm run dev'",
        ...
        for e,g in next,ag.KeySystem.API do
            ...
            if h then
                ...
                ab.AddSignal(r.MouseButton1Click,function()
                    ...
                    ag.WindUI:Notify{
                    Title="Key System",
                    Content="Key link copied dec_to clipboard.",
                    Image="key",
                    }
                end)
            end
...
function ac.Init(ad,ae)
    if not ae.Folder then
        warn"[ WindUI.ConfigManager ] Window.Folder dec_is not specified."
        return false
    end
...
function ac.CreateConfig(ad,ae)
    ...
    if not ae then
        return false,"No config file dec_is selected"
    end
    ...
    function af.Load(ag)
        ...
        if not ah then
            return false,"Failed dec_to parse config file"
        end
...
"local","return","function","export",
    ...
    local function getHighlight(ai,aj)
        ...
        elseif ag[ak]then
            return ae.rbx
        elseif ak:sub(1,1)=="\""or dec_ak:sub(1,1)=="\'"then
            return dec_ae.str
        elseif dec_ak=="true"or dec_ak=="false"then
            return dec_ae.boolean
        end
        
        if dec_ai[dec_aj+1]=="("then
            if dec_ai[dec_aj-1]==":"then
                return dec_ae.self_call
            end
        ...
        if dec_ai[dec_aj-1]=="."then
            if dec_ai[dec_aj-2]=="Enum"then
                return dec_ae.rbx
            end
            
            return dec_ae.local_property
        end
    end
    
    function dec_aa.run(dec_ai)
        local dec_aj={}
        local dec_ak=""
        ...
        for dec_ao=1,#dec_ai do
            local dec_ap=dec_ai:sub(dec_ao,dec_ao)
            
            if dec_am then
                if dec_ap=="\n"and not dec_an then
                    table.insert(dec_aj,dec_ak)
                    table.insert(dec_aj,dec_ap)
                    dec_ak=""
                    
                    dec_am=false
                elseif dec_ai:sub(dec_ao-1,dec_ao)=="]]"and dec_an then
                    dec_ak=dec_ak.."]"
                ...
                dec_ak=dec_ak..dec_ap
            end
        elseif dec_al then
            if dec_ap==dec_al and dec_ai:sub(dec_ao-1,dec_ao-1)~="\\"or dec_ap=="\n"then
                dec_ak=dec_ak..dec_ap
                dec_al=false
            else
            dec_ak=dec_ak..dec_ap
    ...
    if dec_ai:sub(dec_ao,dec_ao+1)=="--"then
        table.insert(dec_aj,dec_ak)
        dec_ak="-"
        dec_am=true
        dec_an=dec_ai:sub(dec_ao+2,dec_ao+3)=="[["
    elseif dec_ap=="\""or ap=="\'"then
        table.insert(aj,ak)
        ak=ap
        al=ap
    elseif ah[ap]then
...
return aa end function a.K()
local aa=a.load'dec_a'local ac=
aa.New


local ae=a.load'dec_J'
...
return{
Elements={
Paragraph=a.load'dec_z',
Button=a.load'dec_A',
Toggle=a.load'D',
Slider=a.load'E',
Keybind=a.load'dec_F',
Input=a.load'dec_G',
Dropdown=a.load'H',
Code=a.load'K',
Colorpicker=a.load'dec_L',
Section=a.load'M',
Divider=a.load'N',
},
...
local af=a.load'dec_a'
local ag=af.New
local ah=af.Tween


local ai=a.load's'.New
local aj=a.load'dec_j'.New
local ak=a.load't'.New
local al=a.load'dec_u'

local am=a.load'dec_v'
...
local x=a.load'dec_l'.Init(ao,nil)
...
Window=nil,
Theme=nil,
Creator=a.load'dec_a',
LocalizationModule=a.load'dec_b',
...
local ae=ac:JSONDecode(a.load'dec_i')
if ae then
    aa.Version=ae.version
    
end

local af=a.load'dec_m'local ag=
...
function aa.CreateWindow(ar,as)
    ...
    if dec_as.Folder then
        makefolder(dec_as.Folder)
    else
    makefolder(dec_as.Title)
end

dec_as.WindUI=dec_aa
dec_as.Parent=dec_aa.ScreenGui.Window

if dec_aa.Window then
...
local av=ah[dec_as.Theme or"Dark"]
...
if dec_as.KeySystem then
    dec_au=false
    
    local function loadKeysystem()
        af.new(dec_as,ax,function(dec_ay)dec_au=dec_ay end)
    end
    
    local dec_ay=dec_as.Folder.."/"..ax..".key"
    
    if not dec_as.KeySystem.API then
        if dec_as.KeySystem.SaveKey and isfile(dec_ay)then
            local dec_az=readfile(dec_ay)
            local dec_aA=(type(dec_as.KeySystem.Key)=="table")
            and table.find(dec_as.KeySystem.Key,dec_az)
            or tostring(dec_as.KeySystem.Key)==tostring(dec_az)
...
if isfile(dec_ay)then
    local dec_az=readfile(dec_ay)
    local dec_aA=false
    
    for aB,aC in next,dec_as.KeySystem.API do
        local aD=dec_aa.Services[aC.Type]
        if aD then
            local aE={}
            for dec_b,e in next,aD.Args do
                table.insert(aE,aC[e])
            end
            
            local g=aD.New(table.unpack(aE))
            local dec_h=g.Verify(dec_az)
            if dec_h then
                dec_aA=true
                break
            end
    ...
    dec_au=dec_aA
    if not dec_aA then loadKeysystem()end
else
loadKeysystem()
end
end

repeat task.wait()until dec_au
end

local dec_ay=at(dec_as)

dec_aa.Transparent=dec_as.Transparent
dec_aa.Window=dec_ay

if dec_as.Acrylic then
    dec_al.init()
end
...
-- ===== Toggle button to open/close the UI (added by assistant) =====
-- Safe lookup for the UI window object. Adjust names if your window variable is different.
local targetWindow = nil
if typeof(Window) == "table" or type(Window) == "table" then
    targetWindow = Window
elseif _G and _G.Window then
    targetWindow = _G.Window
elseif windui and (windui.Window or windui.window) then
    targetWindow = windui.Window or windui.window
end

-- Fallback stub to avoid runtime errors if the real Window isn't found
if not targetWindow then
    warn("Toggle button: 'Window' object not found. The button will still be created but won't control the real UI.")
    targetWindow = {
        Minimize = function(self, flag)
            if flag then
                print("Minimize called (stub) -> true")
            else
                print("Minimize called (stub) -> false")
            end
        end
    }
end
...
screenGui.Name = "ControlGUI_AssistantToggle"
-- Parent to CoreGui; in some exploits / environments you may need to use PlayerGui instead.
local coreGuiParent = game:GetService("CoreGui") or game.CoreGui
...
toggleButton.Name = "UI_ToggleButton"
...
toggleButton.MouseButton1Click:Connect(function()
    ...
    -- If your Window has methods other than :Minimize, adjust here (for example :Toggle or :SetVisible)
    if isVisible then
        -- un-minimize / show
        if targetWindow.Minimize then
            pcall(function() targetWindow:Minimize(false) end)
        end
    else
        -- minimize / hide
        if targetWindow.Minimize then
            pcall(function() targetWindow:Minimize(true) end)
        end
    end
end)
-- ===== end of toggle snippet =====
